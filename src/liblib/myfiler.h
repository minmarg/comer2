...
//functor for reading from a stream into the mystring buffer
struct CopyToBuffer {
    void operator()(char* srcptr, size_t srclen, mystring* buf, size_t*) {
        *buf = mystring(srcptr, srclen);
    }
};
...
int skip_comments( TCharStream* source, mystring& buffer, char cc  = '#');
