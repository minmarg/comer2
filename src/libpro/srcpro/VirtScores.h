...
#ifndef __VirtScores_h__
#define __VirtScores_h__
...
// -------------------------------------------------------------------------
// class VirtScores: Base structure for readable scores
//
class VirtScores
{
public:
...
protected:
    ...
    const float naval_;//value for NA field
...
//  noplvs: number of probability level values
//  nolevels: number of level values of eff. no. sequences
inline
void VirtScores::NewScores( int noplvs, int nolevels )
{
    int n, m;
    DestroyScores();
    ...
    if( scores_ == NULL )
        throw MYRUNTIME_ERROR("VirtScores::NewScores: Not enough memory.");
    noplvs_ = noplvs;
    notbls_ = nolevels;
...
//  effective number of seqns
inline
extspsl::Pslvector* VirtScores::GetScores( int plvl, int lvl )
{
#ifdef __DEBUG__
    if( plvl < 0 || noplvs_ <= plvl )
        throw MYRUNTIME_ERROR("VirtScores::GetScores: Memory access error.");
...
//  eff. no seqns
inline
const extspsl::Pslvector* VirtScores::GetScores( int plvl, int lvl ) const
{
#ifdef __DEBUG__
    if( plvl < 0 || noplvs_ <= plvl )
        throw MYRUNTIME_ERROR("VirtScores::GetScores: Memory access error.");
...
inline
float VirtScores::GetScore( int row, int col, 
    float fstprb, float secprb, float fstens, float secens ) const
{
    MYMSG( "VirtScores::GetScore", 5 );
    ...
    if( row < 0 || col < 0 || card_ <= row || card_ <= col )
        throw MYRUNTIME_ERROR("VirtScores::GetScore: Memory access error.");

    if( noplvs < 1 )
        throw MYRUNTIME_ERROR("VirtScores::GetScore: No probability levels.");
#endif
    if( prblvs_.GetSize())
    ...
    if( nolvs < 1 )
        throw MYRUNTIME_ERROR("VirtScores::GetScore: "
                              "Levels of eff. number of sequences are not given.");
    ...
    //corresponding to probability level
    plvl = 0;
    if( prblvs_.GetSize()) {
        ...
        if( noplvs <= pi || noplvs <= ppi || ppi < pi ) {
            sprintf( locbuf, "Segment covering probability level value not found: %g vs %g.", 
                     fstprb, secprb );
            throw MYRUNTIME_ERROR("VirtScores::GetScore: " + mystring( locbuf ));
        }
    ...
    if( scos == NULL )
        throw MYRUNTIME_ERROR("VirtScores::GetScore: Null score table.");
