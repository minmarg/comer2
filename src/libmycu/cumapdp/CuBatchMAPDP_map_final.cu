...
// #define CUMAPDP_MAP_FINAL_TESTPRINT //27 //449 //1286 //-1 //0//3024//4899
...
// -------------------------------------------------------------------------
// FinalizeMAPDP_MAP: device code for finalizing dynamic programming in 
...
// dp2alndatbuffers, profile-specific statistics from (previous) phase 1 to 
// be complemented with information from phase 2;
    ...
    char* __restrict__ outalns )
{
    ...
    //        scoreCache[CUMAPDP_2DCACHE_DIM_D+1];
    //cache of maximum scores from the last processed diagonals
    // (add 1 to dimensions to avoid bank conflicts):
    __shared__ float maxscCache[TIMES2(CUMAPDP_2DCACHE_DIM_D)+1];
    __shared__ int maxscndxCache[TIMES2(CUMAPDP_2DCACHE_DIM_D)];//indices
    ...
    __shared__ char outAlnCache[nTDP2OutputAlignmentSSS][CUMAPDP_2DCACHE_DIM_D];
    //
    float maxsc;
    int ndx;
    //
    ...
    maxscCache[threadIdx.x+CUMAPDP_2DCACHE_DIM_D] = CUBDP_Q(0);
    ...
    if( threadIdx.x+CUMAPDP_2DCACHE_DIM_D < dbprolenCache )
        maxscCache[threadIdx.x+CUMAPDP_2DCACHE_DIM_D] = 
            tmpdpdiagbuffers[qpos + dbpos2off-CUMAPDP_2DCACHE_DIM_D];

    __syncthreads();

// if((int)pronr==CUMAPDP_MAP_FINAL_TESTPRINT){
//     printf(" MAPTEST: bid= %u tid= %u: pronr= %u len= %d addr= %u score= %.6f xx= %x\n",
//         blockIdx.x,threadIdx.x,pronr,
//         dbprolenCache,dbprodstCache,maxscCache[threadIdx.x],maxscoordsbuf[dbpos2off]
//     );
//     if( threadIdx.x+CUMAPDP_2DCACHE_DIM_D < dbprolenCache )
//     printf(" MAPTEST: bid= %u tid= %u: pronr= %u len= %d addr= %u score= %.6f xx= %x\n",
//         blockIdx.x,threadIdx.x+CUMAPDP_2DCACHE_DIM_D,pronr,
//         dbprolenCache,dbprodstCache,maxscCache[threadIdx.x+CUMAPDP_2DCACHE_DIM_D],
//         maxscoordsbuf[dbpos2off-CUMAPDP_2DCACHE_DIM_D]
//     );
...
// }
    ...
    dpfinmaxndxinit( maxscCache, maxscndxCache, threadIdx.x, threadIdx.x+CUMAPDP_2DCACHE_DIM_D );
    __syncthreads();
    for(int i = CUMAPDP_2DCACHE_DIM_D >> 1; i >= 1; i >>= 1 ) {
    ...
    if((int)pronr==CUMAPDP_MAP_FINAL_TESTPRINT){
        printf(" MAP Fin: bid= %u tid= %u: pronr= %u len= %d addr= %u maxsc= %.6f ndx= %d  "
            "(%x: y= %d x= %d)\n",
            blockIdx.x,threadIdx.x,pronr,
    ...
    if( threadIdx.x == 0 ) {
        if((int)pronr==CUMAPDP_MAP_FINAL_TESTPRINT){
            printf(" MAP Fin0 (pronr=%u): maxsc= %.6f y= %d x= %d alnlen= %d(%d) "
