...
// #define CUMAPDP_FWD_TESTPRINT 1682 //27 //0 //3024//4899
    ...
    float* __restrict__ tmpfwdprobs )
{
    ...
    //
    float maxlogprobCache;//maximum log probability
    //
...
//     __syncthreads();
    ...
    if( 0 <= x-1 && x < (int)dbprolenCache ) {
        ...
        maxlogprobCache = tmpdpdiagbuffers[qpos + dbpos2 - 1];
    }
    else {
        DPLocInitCache<CUMAPDP_2DCACHE_DIM_D>(diag1Cache, 0/*shift*/, MAPDPLocGetLogProb0());
        DPLocInitCache<CUMAPDP_2DCACHE_DIM_D>(diag2Cache, 1/*shift*/, MAPDPLocGetLogProb0());
        maxlogprobCache = CUBDP_Q(0);
    }
    ...
    for( int i = 0; i < ilim/*CUMAPDP_2DCACHE_DIM_X*/; i++ ) {
        float val1, val2, val3, val4, val5, max;
        //
        ...
        //padding causes conflict-free access of scores
        val5 = scoreCache[threadIdx.x][i];
        ...
        max = myhdmax/*fmaxf*/( val1, val2 );
        val1 = MAPDPexpf(val1-max) + MAPDPexpf(val2-max);
        val1 = MAPDPlogf(val1) + max;
...
#ifdef CUMAPDP_FWD_TESTPRINT
        if(pronr==CUMAPDP_FWD_TESTPRINT){
...
#ifdef CUMAPDP_FWD_TESTPRINT
        if(pronr==CUMAPDP_FWD_TESTPRINT)
            printf(" >>> d=%u(%u) s=%u (t%02u): MAX= %.6f wrt= %d xpos= %d\n", 
                    blkdiagnum,lastydiagnum,blockIdx.x,threadIdx.x, 
                    maxlogprobCache, x+CUMAPDP_2DCACHE_DIM_X-1<(int)dbprolenCache, 
    ...
    if( 0 <= x+CUMAPDP_2DCACHE_DIM_X-1 && x+CUMAPDP_2DCACHE_DIM_X-1 < (int)dbprolenCache ) {
        DPLocWriteBuffer<CUMAPDP_2DCACHE_DIM_D>( pdiag1, tmpdpdiagbuffers, 
                          dbpos2+CUMAPDP_2DCACHE_DIM_X-1, 0, 
                          dblen2 );
        DPLocWriteBuffer<CUMAPDP_2DCACHE_DIM_D>( pdiag2, tmpdpdiagbuffers, 
                          dbpos2+CUMAPDP_2DCACHE_DIM_X-1, 
                          dpdssDiag2 * nTDPDiagScoreSubsections * dblen2, 
                          dblen2,
                          1/*shift*/);
    }
    ...
    //write the buffer of maximum log probabilities
    if( 0 <= x+ndx && x+ndx < (int)dbprolenCache ) {
        qpos = dpdssDiagM * nTDPDiagScoreSubsections * dblen2;
        //tmpdpdiagbuffers[qpos + dbpos2+ndx] = maxscCache[threadIdx.x];
        if( CUMAPDP_2DCACHE_DIM_D <= y ) {
            if( tmpdpdiagbuffers[qpos + dbpos2+ndx] < maxlogprobCache ) {
                tmpdpdiagbuffers[qpos + dbpos2+ndx] = maxlogprobCache;
            }
        } else {
            tmpdpdiagbuffers[qpos + dbpos2+ndx] = maxlogprobCache;
        }
    ...
    if( 0 <= x && x < (int)dbprolenCache )
        DPLocWriteBuffer<CUMAPDP_2DCACHE_DIM_X>( 
            bottmCache, tmpdpbotbuffer, dbpos2, 0, dblen2 );

    //write log forward probabilities written in scoreCache for SMEM reuse;
    ...
    for( int i = 0; i < CUMAPDP_2DCACHE_DIM_D; i++ ) {
        ...
        if( 0 <= y-i && y-i < (int)nqyposs && 0 <= x+i && x+i < (int)dbprolenCache ) {
            ...
            qpos = (y-i) * dblen2 + i;
            tmpfwdprobs[qpos + dbpos2] = scoreCache[i][threadIdx.x];
        }
    }
}
